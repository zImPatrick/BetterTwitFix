...
testMultiMedia_compare={'tweet': 'https://twitter.com/Twitter/status/1293239745695211520','description': 'We tested, you Tweeted, and now we’re rolling it out to everyone! https://t.co/w6Q3Q6DiKz', 'screen_name': 'Twitter', 'type': 'Multi', 'images': [{"type": "Image","url": "https://pbs.twimg.com/media/EfJ-C-JU0AAQL_C.jpg"},{"type": "Image","url": "https://pbs.twimg.com/media/EfJ-aHlU0AAU1kq.jpg"},{},{},"2"], 'time': 'Tue Aug 11 17:35:57 +0000 2020', 'qrtURL': None, 'nsfw': False, 'verified': True, 'size': {}}
...
def compareDict(original,compare):
    for key in original:
        ...
        if type(compare[key]) is not dict:
            assert compare[key]==original[key], f"{key} does not match: {compare[key]} != {original[key]}"
        else:
...
def test_combine():
    ...
    img1 = twt["images"][0]["url"]
    img2 = twt["images"][1]["url"]

    resp= client.get(testMultiMediaTweet.replace("https://twitter.com",""),base_url="https://c.vxlocalhost",headers={"User-Agent":"test"})
    assert resp.status_code == 200
    assert f"{img1},{img2}" in str(resp.data)
